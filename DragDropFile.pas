    ...
    begin
      ...
      try
        ...
        try
          ...
          Stream.Seek(0, STREAM_SEEK_SET, ps);

          while (Remaining > 0) do
          begin
            if (Failed(Stream.Read(pChunk, Remaining, pFIxedUInt(@Size)))) or
              (Size = 0) then
              break;
            inc(pChunk, Size);
            dec(Remaining, Size);
        ...
        if (AMedium.hGlobal <> 0) then
          try
            try
              ...
              try
                OleCheck(Stream.Seek(0, STREAM_SEEK_SET, ps));
                OleCheck(Stream.Read(Data, StatStg.cbSize, PFixedUInt(@ReadSize)));
