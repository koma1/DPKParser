...
function TFixedStreamAdapter.Read(pv: Pointer; cb: FixedUInt;
...
begin
  ...
  if (not FHasSeeked) then
    Seek(0, STREAM_SEEK_SET, sp);
  Result := inherited Read(pv, cb, pcbRead);
end;

function TFixedStreamAdapter.CopyTo(stm: IStream; cb: LargeUInt;
  out cbRead: LargeUInt; out cbWritten: LargeUInt): HResult;
const
  ...
  try
    ...
    try
      ...
      begin
        ...
        BytesWritten := BytesWritten + Integer(BurstWritten);
        if (Succeeded(Result)) and (Integer(BurstWritten) <> BurstWriteSize) then
          Result := E_FAIL;
        if (Failed(Result)) then
          Exit;
        Dec(cb, Integer(BurstWritten));
      end;
    finally
      FreeMem(Buffer);
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if (Stream <> nil) then
        begin
          Stream.Seek(0, STREAM_SEEK_SET, sp);
          Result := True;
          Remaining := Size;
          pChunk := Buffer;
          ...
          try
            ...
            try
              ...
              begin
                ...
                Result := (Succeeded(Stream.Read(ChunkBuffer, Chunk, PFixedUINT(@Chunk))));
                if (Chunk = 0) then
                  break;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while (Result) and (Remaining > 0) do
        begin
          Result := (Succeeded(Stream.Read(Buffer, Remaining, pFixedUInt(@Chunk))));
          if (Chunk = 0) then
            break;
  ...
  *)
  ...
  begin
    ...
    try
      ...
      Stream.Seek(0, STREAM_SEEK_END, ps);
